...
void EEPROM_init( void ) {
	...
	SysTick_Init();							/////////////////////////////////////////////////////
	RCC->AHB2ENR |= (EEPROM_PORT_CLOCK);    	// GPIOB: EEPROM SCL/SDA
	...
	for (uint32_t i = 0; i < EEPROM_NUM ; i++) {
		...
		EEPROM_PORT->OTYPER &= ~(0x1 << pin);//PP mode
		...
		//EEPROM_PORT->PUPDR |= (0x2 << (pin * 2));// PD resistor enable
	...
	I2C1->TIMINGR = 0x00000509;            // 16 MHz SYSCLK timing from CubeMX				// 100 kHz
...
void EEPROM_write (uint16_t address ) {
	...
	/* USER wait for I2C_ISR_TXE to clear before writing each Byte, e.g. ... */
	while(!(I2C1->ISR & I2C_ISR_TXE)) ;   // wait for start condition to transmit
	I2C1->TXDR = (EEPROM_MEMORY_ADDR >> 8); // xmit MSByte of address
	while(!(I2C1->ISR & I2C_ISR_TXE)) ;   // wait for start condition to transmit
	I2C1->TXDR = (EEPROM_MEMORY_ADDR & 0xFF); // xmit LSByte of address
