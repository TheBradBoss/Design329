...
void LCD_write_power(uint16_t millivolt) {
	__disable_irq();
   // Move cursor to the position after "Power: "
   LCD_command(RETURN_HOME);  // Set cursor to home position
   uint8_t digits[3]; // Array to store each digit
   // Extract digits
   for (uint8_t i = 0; i < 3; i++) {
       digits[i] = millivolt % 10;
       millivolt /= 10;
   }
   LCD_command(0x87);
   // Print the digits in reverse order
   LCD_write_char(digits[2] + '0'); // Hundreds place
   LCD_write_char(digits[1] + '0'); // Tens place
   LCD_write_char(digits[0] + '0'); // Ones place
   // Print " W" after the value
   LCD_write_string(" W");
   __enable_irq();
}
void LCD_write_volt(uint16_t millivolt) {
	__disable_irq();
   // Takes milliseconds and outputs time on LCD (N.SSS). Prints right to left
   LCD_command(CURSOR_LEFT);                 // Cursor moves left
   LCD_command(CURSOR_LEFT);                 // Cursor moves left
   LCD_command(0xCD);
   LCD_write_string("h/pm");
   uint8_t digits[4]; // Array to store each digit
   // Extract digits
   ...
   LCD_write_char(digits[0] + '0'); // Tens place
   LCD_write_char('.');             // Decimal point
   LCD_write_char(digits[1] + '0'); // Ones place
   LCD_write_char(digits[2] + '0'); // Tenths place
...
//		LCD_write_char( (millivolt % 10) + '0' );	// Print each digit
//		if ( i == 2 ) LCD_write_char( '.' );	// Add dot between s/ms
//		millivolt /= 10;							// Next left digit
//	}
//	LCD_command(CURSOR_RIGHT);
//	;
//}
